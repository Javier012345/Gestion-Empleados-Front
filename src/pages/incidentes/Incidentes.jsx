...
const Incidentes = () => {
    ...
    const filteredIncidentes = incidentes.filter(incidente => {
        ...
        const searchMatch = filters.searchQuery ? incidente.empleados_involucrados.some(emp => 
            emp.nombre.toLowerCase().includes(searchLower) ||
            emp.apellido.toLowerCase().includes(searchLower) ||
            String(emp.dni).includes(searchLower)
        ) : true;

        const fechaIncidente = new Date(incidente.fecha_ocurrencia);
        const mesMatch = filters.mes ? (fechaIncidente.getUTCMonth() + 1) === parseInt(filters.mes) : true;
        const anioMatch = filters.anio ? fechaIncidente.getUTCFullYear() === parseInt(filters.anio) : true;
        const estadoMatch = filters.estado ? incidente.estado === filters.estado : true;

        return searchMatch && mesMatch && anioMatch && estadoMatch;
    });
            ...
            <form className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm mb-6 border border-gray-200 dark:border-gray-700">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 items-end">
                    <div>
                        ...
                        <input type="text" name="searchQuery" value={filters.searchQuery} onChange={handleFilterChange} placeholder="Nombre o DNI..." className="mt-1 w-full pl-4 pr-4 py-2 rounded-lg border bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="estado" value={filters.estado} onChange={handleFilterChange} className="mt-1 w-full pl-4 pr-4 py-2 rounded-lg border bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white">
