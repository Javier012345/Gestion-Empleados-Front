...
import { getSancionesByEmpleadoId, getEmpleadoById } from '../../services/api';
import { Loader, ShieldAlert, ArrowLeft, Calendar, CalendarCheck, ArrowRight, Shield, Filter, RotateCw } from 'lucide-react';

const SancionesEmpleado = () => {
    const { id } = useParams();
    ...
    const [filters, setFilters] = useState({ month: '', year: '', tipo: '' });

    const availableYears = useMemo(() => [...new Set(sanciones.map(s => new Date(s.fecha_sancion).getFullYear()))].sort((a, b) => b - a), [sanciones]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const [sancionesRes, empleadoRes] = await Promise.all([
                    getSancionesByEmpleadoId(id),
                    getEmpleadoById(id)
                ]);
                setSanciones(sancionesRes.data);
                setFilteredSanciones(sancionesRes.data);
                setEmpleado(empleadoRes.data);
                setError(null);
            } catch (err) {
                setError("No se pudieron cargar las sanciones para este empleado.");
                console.error(err);
            } finally {
                setLoading(false);
            }
    ...
    useEffect(() => {
        const applyFilters = () => {
            const filtered = sanciones.filter(sancion => {
                const fecha = new Date(sancion.fecha_sancion);
                ...
                return monthMatch && yearMatch && tipoMatch;
            });
            ...
            <form className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm mb-6 border border-gray-200 dark:border-gray-700">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-end">
                    ...
                    <div>
                        ...
                        <select name="tipo" value={filters.tipo} onChange={handleFilterChange} className="mt-1 w-full pl-4 pr-4 py-2 rounded-lg border bg-white border-gray-300 text-gray-900 focus:ring-red-500 focus:border-red-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            ...
            {filteredSanciones.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredSanciones.map(sancion => (
                        <div key={sancion.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-5 flex flex-col border dark:border-gray-700 hover:shadow-xl hover:border-red-500/20 transition-all duration-300 group">
                            <div className="flex-1">
                                ...
                                <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400 mb-4">
                                    <div className="flex items-center gap-2"><Calendar size={14} /><span>{formatDate(sancion.fecha_inicio)}</span></div>
                                    {sancion.fecha_fin && <div className="flex items-center gap-2"><CalendarCheck size={14} /><span>{formatDate(sancion.fecha_fin)}</span></div>}
                                </div>
                            </div>
                            <div className="border-t dark:border-gray-700 pt-4 flex justify-between items-center">
                                <p className="text-xs text-gray-500 dark:text-gray-400">Por: {sancion.responsable.nombre} {sancion.responsable.apellido}</p>
                ...
                <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <div className="mx-auto h-12 w-12 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center mb-4"><Shield className="h-6 w-6 text-gray-400" /></div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">Sin Sanciones</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{sanciones.length > 0 ? 'No hay sanciones que coincidan con los filtros.' : 'Este empleado no tiene sanciones registradas.'}</p>
                </div>
            )}
        </div>
    );
